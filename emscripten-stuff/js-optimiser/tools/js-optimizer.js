...
function eliminate(ast, memSafe) {
  ...
  traverseGeneratedFunctions(ast, function(func, type) {
    ...
    function track(name, value, defNode) { // add a potential that has just been defined to the tracked list, we hope to eliminate it
      ...
      printErr("tracking: " + name);
      traverse(value, function(node, type) {
        if (type == 'name') {
          if (!ignoreName) {
            var name = node[1];
            if (!(name in locals)) {
              printErr("  uses name " + name + " which is a global");
              usesGlobals = true;
            }
    ...
    function invalidateGlobals() {
      ...
      for (var i = 0; i < temp.length; i++) {
        printErr("Not eliminating " + temp[i] + " due to global usage");
    ...
    function invalidateMemory() {
      ...
      for (var i = 0; i < temp.length; i++) {
        printErr("Not eliminating " + temp[i] + " due to memory usage");
        delete tracked[temp[i]];
      }
    }
    function invalidateByDep(dep) {
      ...
      for (var i = 0; i < temp.length; i++) {
        printErr("Not eliminating " + temp[i] + " due to dep");
        delete tracked[temp[i]];
      }
    }
    function invalidateCalls() {
      ...
      for (var i = 0; i < temp.length; i++) {
        printErr("Not eliminating " + temp[i] + " due to calls");
        delete tracked[temp[i]];
      }
    }
